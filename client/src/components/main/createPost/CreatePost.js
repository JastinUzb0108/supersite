...
function CreateProduct() {
    ...
    const handleSubmit = async e =>{
        ...
        try {
            if(onEdit){
                await axios.put(`/api/post/${post._id}`, {...post, userID: users, media: media, type1: type1, type2: type2, type3: type3}, {
                    headers: {Authorization: token}
                })
            }else{
                await axios.post('/api/post', {...post, userID: users, media: media, type1: type1, type2: type2, type3: type3}, {
                    headers: {Authorization: token}
                })
            }
    ...
    return (
        <div className="create_product confirest">
            <div className="upload_imgs debegs">
                    <div className="upload_cart">
                        <input type="file" name="file" id="files_up" onChange={handleUpload} />
                        {
             ...
             <div className="constant">
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="row">
                        ...
                        <input type="text" name="titleEng" id="titleEng" required 
                        value={post.titleEng} onChange={handleChangeInput} />
                    </div>

                    <div className="row">
                        <label htmlFor="links">Website Link</label>
                        <input type="url" name="links" id="links" required 
                        value={post.links} onChange={handleChangeInput} />
                    </div>

                    <div className="row">
                        <textarea type="text" name="descriptionRu"  id="descriptionRu" required  placeholder="ContentRu"
                        value={post.descriptionRu}  rows="5" onChange={handleChangeInput} />
                    </div>
                    <div className="row">
                        <textarea type="text" name="descriptionEn"  id="descriptionEn" required  placeholder="ContentEng"
                        value={post.descriptionEn}  rows="5" onChange={handleChangeInput} />
                    </div>
